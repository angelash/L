...
namespace Mogo.RPC
{
    ...
    public class DefParser : DataLoader
    {
        ...
        {
            var propList = new Dictionary<ushort, EntityDefProperties>();
            if (props != null)
            {
                short index = -1; //属性索引
                for (var i = 0; i < props.Count; i++) //遍历实体的所有属性
                {
                    var prop = ((Dictionary<object, object>)props[i]).First();
                    List<object> flag = null;
                    List<object> node = null;
                    foreach (var item in (List<object>)prop.Value) //遍历属性的字段
                    {
                        var arg = (List<object>)item;
        ...
        private void GetHmfClientMethods(EntityDef etyDef, List<object> methods)
        {
            ...
            if (methods != null)
            {
                ...
                for (var i = 0; i < methods.Count; i++)
                {
                    ...
                    var fun = (Dictionary<object, object>)methods[i];
                    var kv = fun.First();
            ...
            if (methods != null)
            {
                ...
                for (var i = 0; i < methods.Count; i++)
                {
                    ...
                    if (v != null && v.Count != 0 && (string)((List<object>)v[0])[0] == EL_EXPOSED)
                    {
                        ...
                        m.FuncID = index;
                        m.ArgsType = new List<VObject>();
                        for (var j = 1; j < v.Count; j++) //跳过第一个Tag字段
                        {
                            var args = (List<object>)v[j];
        ...
        private readonly Dictionary<uint, EntityDef> m_entitysByID;

        #endregion

        #region 公有属性

        public static DefParser Instance { get; private set; }

        public Dictionary<String, EntityDef> EntitysByName { get; private set; }
        ...
        static DefParser()
        {
            Instance = new DefParser();
        }

        private DefParser()
        {
            EntitysByName = new Dictionary<string, EntityDef>();
        ...
        public void InitEntityData(String defineFilePath, String defineListFileName)
        {
            try
            {
                ...
                var xmlRoot = xmlDoc.Children;
                if (xmlRoot != null && xmlRoot.Count != 0)
                {
                    EntitysByName.Clear();
                    for (var i = 0; i < xmlRoot.Count; i++)
                    {
                        var element = xmlRoot[i] as SecurityElement;
                        ParseEntitiesXmlFile(String.Concat(defineFilePath, element.Tag, m_fileExtention), element.Tag,
                            (ushort)(i + 1), !String.IsNullOrEmpty(element.Text));
                    }
        ...
        {
            EntityDef result;
            EntitysByName.TryGetValue(name, out result);
        ...
        private void ParseEntitiesXmlFile(String fileName, String etyName, ushort entityID, bool needMD5 = false)
        {
            ...
            var xmlDoc = XMLParser.LoadXML(strContent);
            if (xmlDoc == null)
                return;
        ...
        {
            var xmlDoc = XMLParser.LoadXML(xml);
            ParseEntitiesXml(xmlDoc, etyName, entityID);
        }
