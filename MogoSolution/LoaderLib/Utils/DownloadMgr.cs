...
public class DownloadMgr
{
                    ...
                    if (e.Message.Contains("Disk full"))
                    {
                        ...
                        if (e.Message.Contains("timed out") || e.Message.Contains("Error getting response stream"))
                        {
                            ...
                            else
                            {
                                ...
                                //again();
                                LoggerHelper.Error("HandleNetworkError:"+e.Message);
    ...
    private static void ShowMsgForUnknown(Action again, string msg = "")
    {
        ...
        ForwardLoadingMsgBoxLib.Instance.ShowMsgBox(languageData.Get(50).content, languageData.Get(7).content,
            languageData.Get(52).content, //网络异常
            (isOk) =>
            {
                if (isOk)
                {
    ...
    public byte[] DownLoadData(String url)
    {
        try
        {
            if (IsEditor)
            {
                ...
                return _webClient.DownloadData(GetRandomParasUrl(url));
            }
            else
            {
                return DownloadGzipData(GetRandomParasUrl(url));
            }
        }
        catch (Exception ex)
        {
    ...
    public string DownLoadText(String url)
    {
        try
        {
            ...
            {
                // 使用DownloadGzipString这个函数无法调试,所以Editor使用这个
                return _webClient.DownloadString(GetRandomParasUrl(url));
            }
            else
            {
                return DownloadGzipString(GetRandomParasUrl(url));
            }
        }
        catch (Exception ex)
        {
    ...
    public static bool DownloadGzipFile(string url, String localPath)
    {
        try
        {
            var content = DownloadGzipString(url);
            if (!String.IsNullOrEmpty(content))
            {
                XMLParser.SaveText(localPath, content);
                return true;
            }
            else
            {
    ...
    public static byte[] DownloadGzipData(string url)
    {
        ...
        {
            Debug.LogError("DownloadGzipData Error:url=" + url + ",error=" + e.Message);
            return null;
        }
    }
    ...
    {
        return Encoding.UTF8.GetString(DownloadGzipData(url));
    }
...
public class DownloadMgrForWeb
{
    public static void DownloadText(String url, Action<string> callback)
    {
        LoggerHelper.Info(url);
        DriverLib.Instance.StartCoroutine(DoDownloadText(url, callback));
    }

    private static IEnumerator DoDownloadText(String url, Action<string> callback)
    {
        var www = new WWW(url);
        yield return www;
        if (callback != null)
    ...
    {
        LoggerHelper.Info(url);
        DriverLib.Instance.StartCoroutine(DoDownloadData(url, callback));
    }

    private static IEnumerator DoDownloadData(String url, Action<byte[]> callback)
    {
        var www = new WWW(url);
        yield return www;
        if (callback != null)
    ...
    public static void DownloadData(String url, string fileName, int version, Action<byte[]> callback)
    {
        LoggerHelper.Info(url);
        DriverLib.Instance.StartCoroutine(DoDownloadData(url, fileName, version, callback));
    }

    private static IEnumerator DoDownloadData(String url, string fileName, int version, Action<byte[]> callback)
    {
        var www = WWW.LoadFromCacheOrDownload(url, version);
        yield return www;
        if (www.assetBundle != null)
        {
            var file = www.assetBundle.Load(fileName, typeof(TextAsset)) as TextAsset;
