...
namespace Mogo.Util
{
    ...
    public static class Utils
    {
        ...
        /// <returns></returns>
        public static string GetTimeForHMS(int sec)
        {
            if (sec <= 0) return "0";
            int hour = sec / 3600;
        ...
        public static Dictionary<String, String> ParseMap(this String strMap, Char keyValueSpriter = KEY_VALUE_SPRITER, Char mapSpriter = MAP_SPRITER)
        {
            ...
            bool hasKeyValue = strMap.IndexOf(keyValueSpriter) > -1;
        ...
        /// 将object转换为对应类型的值。
        /// </summary>
        /// <param name="value">传入的obejct</param>
        /// <param name="type">值的类型</param>
        /// <returns>对应类型的值</returns>
        public static object GetObjectValue(object value, Type type)
        {
            //UnityEngine.Debug.Log(type.Name);
            if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Dictionary<,>))
            {
                //Type[] types = v.GetType().GetGenericArguments();

                var result = type.GetConstructor(Type.EmptyTypes).Invoke(null);
                Type[] types = type.GetGenericArguments();
                foreach (var item1 in (value as Dictionary<object, object>))
                {

                    type.GetMethod("Add").Invoke(result, new object[] { item1.Key, GetObjectValue(item1.Value, types[1]) });
                }
                return result;
            }
            else if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(List<>))
            {
                ...
                foreach (var item1 in (value as List<object>))
                {

                    type.GetMethod("Add").Invoke(result, new object[] { GetObjectValue(item1, types[0]) });
                }
            ...
            else if (type.BaseType == typeof(Enum))
            {
                ...
                //prop.SetValue(t,Enum.ToObject(prop.PropertyType,v),null);
        ...
        public static void CompressDirectory(string sourcePath, string outputFilePath, int zipLevel = 0, bool needPwd = true)
        {
            ...
            compressed.CompressDirectory(sourcePath, zipLevel, needPwd);
        }
        ...
        public static void DecompressToMogoFile(string zipFilePath)
        {
            if (File.Exists(zipFilePath))
            {
                ...
                {
                    ZipEntry entry;
                    decompressor.Password = Utils.GetResNumber().PackArray();
                    ...
                    while ((entry = decompressor.GetNextEntry()) != null)
                    {
                        ...
                        {
                            re.Write(data, 0, bytesRead);
                        }
                        byte[] depress = re.ToArray();// CompressBytes(re.ToArray(), filePath, 5);

                        var info = MogoFileSystem.Instance.BeginSaveFile(filePath, entry.Size);
                        MogoFileSystem.Instance.WriteFile(info, depress, 0, depress.Length);
        ...
        public static void CompressDirectory(this Stream target, string sourcePath, int zipLevel, bool needPwd)
        {
            ...
            using (ZipOutputStream compressor = new ZipOutputStream(target))
            {
                ...
                if (needPwd)
                    compressor.Password = Utils.GetResNumber().PackArray();
        ...
        public static byte[] CompressBytes(Byte[] data, string filePath, int zipLevel)
        {
            MemoryStream re = new MemoryStream();
            using (ZipOutputStream compressor = new ZipOutputStream(re))
            {
                compressor.SetLevel(zipLevel);

                var file = filePath.StartsWith(@"\") ? filePath.ReplaceFirst(@"\", "") : filePath;
                file = file.Replace(@"\", "/");
                compressor.PutNextEntry(new ZipEntry(file));
        ...
        public static byte[] DecompressBytes(byte[] zipMemory)
        {
        ...
        public static string CreateMD5(string str)
        {
            ...
            byte[] outbytes = CreateMD5(inbytes);
            return FormatMD5(outbytes);
        }
        ...
        {//这里千万不能随便修改明文内容，安卓看到就完蛋了，仅允许编辑器和ios用明文
#if UNITY_EDITOR || UNITY_IPHONE || UNITY_WEBPLAYER
        ...
        public static void GetHttp(string Url, Action<string> onDone, Action<HttpStatusCode> onFail)
        {
            ...
            {
                Debug.LogError("GetHttp Error:url=" + Url + ",error=" + e.Message);
            }
        }
    }
}