...
public class LoadResources : MonoBehaviour, ILoadAsset
{
                ...
                if (obj != null) {
                    ...
                    if (go != null) {
						if (sw.ElapsedMilliseconds > 20) {
							LoggerHelper.Info(string.Format("Stopwatch={0},prefab={1},go={2}", sw.ElapsedMilliseconds, prefab, go.name));
						}
                        var clone = go as GameObject;
                        if (clone != null)
                        {
    ...
    public bool Release(string prefab)
    {
    ...
    public void LoadAsset(string prefab, byte priority, Action<Object> loaded, Action<float> progress)
    {
        LoadAsset(prefab, loaded, progress);
    }

    public void LoadUIAsset(string prefab, byte priority, Action<Object> loaded, Action<float> progress)
    {
    ...
    private bool IsResource(string resource)
    {
        ...
        if (resource.EndsWith(".png", StringComparison.OrdinalIgnoreCase) && !resource.Contains("/Characters/Avatar/") && !resource.Contains("/Images/") && !resource.Contains("/Textures/") && !resource.Contains("/Characters/Texture/"))
            return false;
        return true;
    }
    ...
    {
        return m_filesDic != null && m_filesDic.ContainsKey(fileName);
    }
}
...
public class ResourceInfo
{
    ...
    public Object GameObject
    {
        get
        {
            if (m_gameObject == null)
            {
                try
                {
                    m_gameObject = Resources.Load(Path, ResType);
                }
                catch (Exception ex)
                {
    ...
    public System.Collections.IEnumerator GetGameObject(Action<Object> action)
    {
        if (!m_gameObject)
        {
            //try
            //{
            //Debug.LogWarning(ResType);
            var req = Resources.LoadAsync(Path, ResType);
            yield return req;

            //LoggerHelper.Info(string.Format("Load complete path={0}", Path));
            m_gameObject = req.asset;
            if (m_gameObject == null)
