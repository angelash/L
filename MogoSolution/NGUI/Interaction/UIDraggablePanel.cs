...
public class UIDraggablePanel : IgnoreTimeScale
{
    ...
    public delegate void OnMoveNotification(Vector3 delta);
    ...
    public OnMoveNotification onMoving;
    public Action<Vector3> OnMovingAfterMoveRelative;
    public Action<bool> OnPress;
    public Action<Vector2> OnDrag;
    public float mThreshold = 0.0001f;
    ...
    private bool m_use_custom_bouns = false;
	/// <summary>
	/// Panel that's being dragged.
	/// </summary>

    public UIPanel panel { get { return mPanel; } set { mPanel = value;  } }
	...
	public Bounds bounds
	{
		get
		{
            if (!m_use_custom_bouns && !mCalculatedBounds)
			{
	...
	public void MoveRelative (Vector3 relative)
	{
        ...
        if (OnMovingAfterMoveRelative != null)
        {
            OnMovingAfterMoveRelative(relative);
        }
	...
	void LateUpdate ()
	{
        if (mPanel.clipping != UIDrawCall.Clipping.None)
        {
            ...
            if (!Mathf.Approximately(scale.x, scale.z))
            {
                ...
                transform.localScale = new Vector3(local_scale.x, local_scale.y, local_scale.z * scale.x / scale.z);
            }
        }
		...
		if (mShouldMove && !mPressed)
		{
			...
			if (mMomentum.magnitude > mThreshold)
			{
