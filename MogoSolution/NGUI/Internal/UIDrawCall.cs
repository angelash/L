...
public class UIDrawCall : MonoBehaviour
{
    ...
    /// <summary>
    /// 使用动态渲染队列
    /// 先设置队列 编号 接着设置 useDynamicRenderQueue true
    /// </summary>
    public int dynamicRenderQueue = 0;

    private bool _useDynamic = false;
    public bool useDynamicRenderQueue
    {
        get { return _useDynamic; }
        set
        {
            if (_useDynamic != value)
            {
                _useDynamic = value;
                mReset = true;
            }
	...
	public bool depthPass { get { return mDepthPass; } set { if (mDepthPass != value) { mDepthPass = value; } } }
	...
	void UpdateMaterials ()
	{
        ...
        //动态调整材质渲染深度
	    if (useDynamicRenderQueue)
	    {
	        if (mDynamicMaterial == null)
	        {
	            mDynamicMaterial = new Material(mSharedMat);
	            mDynamicMaterial.hideFlags = HideFlags.DontSave;
	            mDynamicMaterial.renderQueue = dynamicRenderQueue;
	        }
	    }
	    else if(mDynamicMaterial != null)
	    {
			NGUITools.Destroy(mDynamicMaterial);
			mDynamicMaterial = null;
	    }
        ...
        if (useClipping)
		{
			...
			if (mClipping != Clipping.None)
			{
                ...
                if (mClipping != Clipping.None) shader = Shader.Find(shaderName + soft);
...
// 		}
	    ...
	    if (useDynamicRenderQueue)
	    {
	        mat = mDynamicMaterial != null? mDynamicMaterial: mSharedMat;
	    }
	    else
	    {
	        mat = (mClippedMat != null) ? mClippedMat : mSharedMat;
	    }
	...
	void OnWillRenderObject ()
	{
		...
		if (mClippedMat != null)
		{
            ...
            mClippedMat.SetVector("_ClipRange", new Vector4(-mClipRange.x / mClipRange.z, -mClipRange.y / mClipRange.w, 1f / mClipRange.z, 1f / mClipRange.w));
            mClippedMat.SetVector("_ClipArgs", sharpness);
		}
	}
	...
	void OnDestroy ()
	{
        ...
        NGUITools.DestroyImmediate(mDynamicMaterial);
