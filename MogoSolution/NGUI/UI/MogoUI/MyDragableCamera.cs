...
public class MyDragableCamera : MonoBehaviour
{
    ...
    public float FViewPortWidth
    {
        get { return m_fPageWidth; }
        set
        {
            m_fViewPortWidth = value;
        }
    }

    float m_fViewPortHeight;
    public float MFViewPortHeight
    {
        get { return m_fViewPortHeight; }
        set { m_fViewPortHeight = value; }
    }
    ...
    void Start()
    {
        ...
        if (!IsSettingViewPortRightNow)
            SettingViewPort();
    }

    private bool IsSettingViewPortRightNow = false;
    public void SettingViewPortRightNow()
    {
        IsSettingViewPortRightNow = true;
        SettingViewPort();
    }

    private void SettingViewPort()
    {
        m_fViewPortWidth = camera.pixelWidth;
        m_fViewPortHeight = camera.pixelHeight;
    }
    ...
    public void SetCurrentPage(int page)
    {
        ...
        {
            if (page < transformList.Count && page>=0)
            {
                m_tp.Reset();
                m_tp.enabled = false;
                ...
                if (page < ListPageDown.Count && page >= 0)
                {
                    ListPageDown[page].SetActive(true);
                }
                else
                {
                    //Debug.LogError("MyDragableCamera.SetCurrentPage() page ERROR:page=" + page + ",ListPageDown size=" + ListPageDown.Count);
                }
                CurrentTransform = page;
            }
            else
            {
                //Debug.LogError("MyDragableCamera.SetCurrentPage() page ERROR:page=" + page + ",transformList size=" + transformList.Count);
            }
        }
        SetArrow();
        SetDOTPage();
    ...
    public void MovePage(Vector2 v)
    {
        ...
        if (IsMovePage)
        {
            ...
            if (CurrentTransform >= 0 && CurrentTransform < ListPageDown.Count)
            {
