...
public class MogoSlider : MonoBehaviour
{
    ...
    {
        if (SliderEndPos == null || SliderBeginPos == null || Slider == null) return;
        float total = SliderEndPos.position.x - SliderBeginPos.position.x;
        float perce = Slider.transform.position.x - SliderBeginPos.position.x;
        float pos = perce / total;
        if (OnSliding != null)
        {
            OnSliding(pos);
        }
