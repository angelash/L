...
{
    public Vector2 LayerVector = new Vector2(2000, 2000);
    ...
    void ResetAffectBySourceCamera()
    {
        ...
        Vector3 tl = m_sourceCamera.WorldToScreenPoint(m_transform.position);
        m_transform.localPosition = new Vector3(width / 2 + center.x, -height / 2 + center.y, 0);
        Vector3 br = m_sourceCamera.WorldToScreenPoint(m_transform.position);
    ...
    // 实际使用的格子列表
    public Dictionary<int, UnityEngine.Component> DataList = new Dictionary<int, UnityEngine.Component>();
    // 缓存格子列表
    private Dictionary<int, UnityEngine.Component> MemoryDataList = new Dictionary<int, UnityEngine.Component>();
    ...
    /// 计算最末尾点的位置
    /// </summary>
    private void CalLastPoint(int NewCount)
    {
        int PageSize = ColSize * RowSize;
        if (IsHorizontal)
        {
        ...
        else
        {
            ...
            {
                float MinY = ((NewCount - 1) / ColSize + 1 - RowSize) * UnitHeight;
                lastPoint = new Vector2(0, -MinY * IsOpposite);
            }
        }
    }
    ...
    private void ShowGrid(int NewCount)
    {
        DataList.Clear();

        int memorySize = MemoryDataList.Count;
        for (int index = 0; index < memorySize; ++index)
        {
            if (MemoryDataList.ContainsKey(index))
            {
                if (MemoryDataList[index] != null && MemoryDataList[index].gameObject != null)
                    MemoryDataList[index].gameObject.SetActive(index < NewCount);

                if (index < NewCount)
                    DataList[index] = MemoryDataList[index];
            }
        }
    }
    ...
    private void ResetPos(Vector3 p)
    {
        ...
        transform.localPosition = LayerVector;
    }
