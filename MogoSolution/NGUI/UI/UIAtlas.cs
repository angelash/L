...
public class UIAtlas : MonoBehaviour
{
	...
	public List<Sprite> spriteList
	{
		...
		set
        {
            ...
            m_dictNameToSprite.Clear();

            for (int i = 0; i < value.Count; ++i)
            {
                m_dictNameToSprite[value[i].name] = value[i];
            }
		}
	}
    ...
    Dictionary<string, Sprite> m_dictNameToSprite = new Dictionary<string, Sprite>();
	public Sprite GetSprite (string name)
	{
		...
		else if (!string.IsNullOrEmpty(name))
		{
            ...
            if (m_dictNameToSprite.Count == 0)
            {
                for (int i = 0, imax = sprites.Count; i < imax; ++i)
                {
                    m_dictNameToSprite[sprites[i].name] = sprites[i];
                }
            }

            //Debug.LogError("TryGetSprite " + name + " " + m_dictNameToSprite.Count);
            if (m_dictNameToSprite.ContainsKey(name))
                return m_dictNameToSprite[name];
		}
		return null;
	}
