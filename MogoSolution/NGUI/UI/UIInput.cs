...
public class UIInput : MonoBehaviour
{
    ...
    /// <summary>
    /// 最大宽度,单位像素,不超过这个值的文字串长度才可以被显示和保存下来
    /// </summary>
	...
	void OnSelect (bool isSelected)
	{
        ...
        if (!isSelected && lostFocusAction!=null)
        {
            lostFocusAction();
        }
	...
	void UpdateLabel ()
	{
	    ...
	    if (maxWidth > 0 && maxChars <= 0 && label.font != null && !label.multiLine)
	    {
	        int oldWidth = label.lineWidth;
            label.lineWidth = maxWidth;

	        mText = label.font.WrapText(mText, label.lineWidth/label.cachedTransform.localScale.x, label.maxLineCount,
	            false, UIFont.SymbolStyle.None);

	        label.lineWidth = oldWidth;
