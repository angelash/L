...
public class UISprite : UIWidget
{
    ...
    public UIAtlas.Sprite GetAtlasSprite()
    {
        ...
        if (mSprite == null && mAtlas != null)
        {
            ...
            if (mSprite != null)
            {
                ...
                {
                    if (material != mAtlas.spriteMaterial)
                    {
 ...
 || !Application.isPlaying && (mOuter != mSprite.outer || mInner != mSprite.inner)
    ...
    private static Dictionary<Texture, Material> BlackWhiteMaterialDic = new Dictionary<Texture, Material>();
    ...
    private static Dictionary<string, Dictionary<Texture, Material>> ColorMaterialDic = new Dictionary<string, Dictionary<Texture, Material>>();
    ...
    private void ShowAsColor(string matName, Material colorMaterial)
    {
        ...
        if (MogoMaterial == null || colorMaterial != MogoMaterial)
        {
            ...
            {
                if (!ColorMaterialDic.ContainsKey(matName))
                {
                    ColorMaterialDic[matName] = new Dictionary<Texture, Material>();
                }
                if (!ColorMaterialDic[matName].ContainsKey(base.material.mainTexture))
                {
                    AssetCacheMgr.GetUIInstance(matName, (resourceName, guid, go) =>
                    {
                        Material mat = go as Material;

                        mat.mainTexture = base.material.mainTexture;
#if UNITY_IPHONE
#else
                        mat.SetTexture("_AlphaTex", base.material.GetTexture("_AlphaTex"));
#endif
                        colorMaterial = mat;
                        MogoMaterial = colorMaterial;
                        ColorMaterialDic[matName][mat.mainTexture] = mat;
                        RefreshPanel(gameObject);
                    });
                }
                else
                {
                    colorMaterial = ColorMaterialDic[matName][base.material.mainTexture];
                    MogoMaterial = colorMaterial;
                    RefreshPanel(gameObject);
                }
            }
            else
            {
                MogoMaterial = colorMaterial;
                Material mat = base.material;
                MogoMaterial.mainTexture = mat.mainTexture;
#if UNITY_IPHONE
#else
                MogoMaterial.SetTexture("_AlphaTex", mat.GetTexture("_AlphaTex"));
#endif
                RefreshPanel(gameObject);
                RefreshPanel(gameObject);
            }
